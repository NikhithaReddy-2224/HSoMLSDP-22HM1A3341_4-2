...
CORS(app, resources={
    r"/*": {
        "origins": [
            "http://localhost:5173",
            "https://hsomlsdp22hm1a3341.vercel.app"
        ],
...
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get(
    'DATABASE_URL',
    'postgresql://postgres:2224@localhost:5432/hsomlsdp_db'
)
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
...
# ✅ Call load_model() here so Railway runs it too
load_model()
...
def get_login_history(user_id):
    ...
    result = [
        {"id": item.id, "email": item.email, "login_time": item.login_time}
        for item in history
    ]
    return jsonify(result)
